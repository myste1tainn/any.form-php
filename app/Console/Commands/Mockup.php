...
class Mockup extends Command {
	 ...
	 * @var string
	 */
	protected $description = 'Mockup data';
	...
	private function mockupParticipants() {
    ...
    private function pad($padToDigit, $num) {
        while (strlen($num) < $padToDigit) {
            $num = '0'.$num;
        }
        return $num;
    }

	private function mockupResults() {
		$forms = [];

        $form1 = Questionaire::find(32);
        // $form1->id = 32;
        $form1->max = 60;

        $form2 = Questionaire::find(33);
        // $form2->id = 33;
        $form2->max = 60;

        $form3 = Questionaire::find(34);
        // $form3->id = 34;
        $form3->max = 30;
